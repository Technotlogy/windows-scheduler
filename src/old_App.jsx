...
function dkey(d){const x=new Date(d);return`${x.getFullYear()}-${String(x.getMonth()).padStart(2,'0')}-${String(x.getDate()).padStart(2,'0')}`}
...
function buildTimeline(block,sleepSettings,appts,jobs,workouts,meals,wakeR,bedR,k,commuteMin=60){
  ...
  if(workouts[k]){const wkStart=wakeEnd+0.25;blocks.push({start:wkStart,end:wkStart+1.5,label:'🏋 Workout',color:'#1a0a0a',textColor:COLORS.workout,border:COLORS.workout,type:'workout'})}
  if(meals[k]){const mpStart=wakeEnd+(workouts[k]?2:0.5);blocks.push({start:mpStart,end:mpStart+1,label:'🍽 Meal Prep',color:'#1a1000',textColor:COLORS.meal,border:COLORS.meal,type:'meal'})}
...
export default function App(){
  ...
  function streak(items,type){
    ...
    for(let i=0;i<365;i++){
      const k=dkey(d)
      if(!items.every((_,idx)=>rchecks[`${k}-${type}-${idx}`]))break
      s++;d=addDays(d,-1)
    }
    return s
  }
  ...
  async function quickAdd(){
    ...
    if(!p){setQuickFb("Couldn't parse. Try again.");setQuickLoad(false);return}
  ...
  function isOD(t){
    ...
    if((t.type==='specific'||t.type==='followup')&&t.date){
      ...
      const dt=new Date(pts[0],pts[1],pts[2])
  ...
  function weekAlerts(){
    ...
    return tasks.filter(t=>{
      ...
      if((t.type==='followup'||t.type==='specific')&&t.date){
        const pts=t.date.split('-').map(Number),dt=new Date(pts[0],pts[1],pts[2])
              ...
              {pipeline.filter(v=>v.stage===stage).map(v=>(
                <div key={v.id} style={{background:'#1e293b',borderRadius:6,padding:'6px 8px',marginBottom:6,fontSize:12}}>
                  ...
                  <div style={{display:'flex',gap:4}}>
                    {stage!=='published'&&<button onClick={()=>setPipeline(p=>p.map(x=>x.id===v.id?{...x,stage:STAGES[STAGES.indexOf(x.stage)+1]}:x))} style={{...S.bs,fontSize:10,padding:'2px 6px',background:STAGE_COLORS[stage]}}>→</button>}
      ...
      <div style={S.ov}><div style={S.mo}>
        ...
        {wf.lifts.map((lft,i)=>(
          <div key={i} style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr auto',gap:5,marginBottom:6,alignItems:'center'}}>
            ...
            <button onClick={()=>setWf(p=>({...p,lifts:p.lifts.filter((_,j)=>j!==i)}))} style={{...S.bs,background:'#7f1d1d',padding:'6px 8px'}}>✕</button>
          </div>
        ))}
